...
Searches recursively for all .git folders and provides per-project tech context.
"""
...
class GitMetricsExtractor:
    ...
    def extract_data(self):
        ...
        return {
            ...
            "primary_tech": self._get_primary_tech() # Replaced global packages with concise tech summary
        }

    def _get_primary_tech(self):
        """Get a concise 2-3 item tech summary for the project breakdown table."""
        tech_indicators = []
        ...
        for root, dirs, files in os.walk(self.repo_path):
            ...
            if 'package.json' in files:
                try:
                    with open(os.path.join(root, 'package.json'), 'r', encoding='utf-8') as f:
                        ...
                        tech_indicators.extend(major_frameworks[:2]) # Max 2 per file
            ...
            if any(f.endswith('.csproj') or f.endswith('.sln') for f in files):
                tech_indicators.append('.NET')
            if any(f == 'go.mod' for f in files):
                tech_indicators.append('Go')

        # Deduplicate and return top 3, or fallback to "Mixed Stack"
...
def create_aggregated_heatmap(all_timelines, start_date, weeks, output_dir, filename):
    """Creates a combined heatmap from multiple repo timelines."""
...
def main():
    ...
    for root, dirs, files in os.walk(args.parent_dir):
        if '.git' in dirs:
            repo_path = root
            dirs.remove('.git') # Prevent walking into .git internals
            ...
            extractor = GitMetricsExtractor(repo_path=repo_path, author_filters=author_filters)
            metrics = extractor.extract_data()
            
            if metrics:
                projects.append({"name": project_name, "metrics": metrics})
                
                if global_first_date is None or metrics["first_date"] < global_first_date:
                    global_first_date = metrics["first_date"]
                if global_last_date is None or metrics["last_date"] > global_last_date:
                    global_last_date = metrics["last_date"]
    ...
    for year in years:
        ...
        create_aggregated_heatmap(
            [p["metrics"]["timeline"] for p in projects], 
            datetime(year, 1, 1), 52, args.output_dir, filename
        )
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        ...
        # CLEAN PROJECT BREAKDOWN (No giant package list)
        f.write("## 📂 Project Breakdown\n\n")
        f.write("| Project | Primary Tech Stack | Commits | Lines Added | Lines Removed |\n")
        f.write("| :--- | :--- | :--- | :--- | :--- |\n")
        for p in projects:
            m = p["metrics"]
            tech = m["primary_tech"]
            f.write(f"| **{p['name']}** | `{tech}` | {m['total_commits']} | +{m['total_insertions']} | -{m['total_deletions']} |\n")
